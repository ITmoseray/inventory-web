...
import {
  ...
  CheckCircle2, Loader2, X, FlaskConical,
} from "lucide-react";
...
function ExpiryBadge({ expiryDate }: { expiryDate: string | null }) {
  ...
  if (status === "expired") {
    return (
      <div className="flex flex-col gap-0.5">
        <Badge className="bg-rose-100 text-rose-700 dark:bg-rose-900/40 dark:text-rose-300 border-0 gap-1 w-fit">
  ...
  if (status === "soon") {
    return (
      <div className="flex flex-col gap-0.5">
        <Badge className="bg-amber-100 text-amber-700 dark:bg-amber-900/40 dark:text-amber-300 border-0 gap-1 w-fit">
  ...
  return (
    <div className="flex flex-col gap-0.5">
      <Badge className="bg-emerald-100 text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-300 border-0 gap-1 w-fit">
        <CheckCircle2 className="h-3 w-3" /> {daysLeft}d left
      </Badge>
      <span className="text-xs text-slate-500">{formatted}</span>
    </div>
  );
...
export default function BatchesPage() {
  ...
  function handleDelete(id: string, batchNumber: string) {
    setDeletingId(id);
    startTransition(async () => {
      try {
  ...
  const expiredCount = batches.filter(
    (b) => b.expiryDate && isPast(new Date(b.expiryDate))
  ).length;
    ...
    <div className="space-y-6 p-6 md:p-8 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <Button
          ...
          className="rounded-2xl bg-indigo-600 hover:bg-indigo-700 text-white font-bold px-5 shadow-lg shadow-indigo-200 dark:shadow-indigo-900/30 transition-all"
        >
        ...
        <div className="p-5 border-b border-slate-100 dark:border-slate-800">
          <div className="relative max-w-sm">
            ...
            <Input
              ...
              className="pl-9 rounded-xl border-slate-200 dark:border-slate-700 dark:bg-slate-800 focus-visible:ring-indigo-500"
            />
          </div>
        </div>
          ...
          <Table>
            ...
            <TableBody>
              {filtered.map((batch) => (
                ...
                >
                  <TableCell>
                    <span className="font-black text-indigo-700 dark:text-indigo-400 tracking-wide">
                      {batch.batchNumber}
                    </span>
                  </TableCell>
                  <TableCell>
                    <div className="flex flex-col">
                      ...
                      {batch.product?.sku && (
                        <span className="text-xs text-slate-400">SKU: {batch.product.sku}</span>
                      )}
                    </div>
                  </TableCell>
                  <TableCell>
        ...
        <DialogContent className="sm:max-w-md rounded-3xl dark:bg-slate-900 border-0 shadow-2xl">
          ...
          <div className="space-y-4 py-2">
            ...
            <div className="space-y-1.5">
              <Label htmlFor="batchNumber" className="font-semibold text-slate-700 dark:text-slate-300">
              ...
              <Input
                ...
                className="rounded-xl dark:bg-slate-800 border-slate-200 dark:border-slate-700 focus-visible:ring-indigo-500"
              />
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <Select
                ...
                onValueChange={(val: string) => setForm((f) => ({ ...f, productId: val }))}
              >
                <SelectTrigger className="rounded-xl dark:bg-slate-800 border-slate-200 dark:border-slate-700 focus:ring-indigo-500">
            ...
            {/* Quantity */}
            <div className="space-y-1.5">
              <Label htmlFor="quantity" className="font-semibold text-slate-700 dark:text-slate-300">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5">
                <Label htmlFor="mfgDate" className="font-semibold text-slate-700 dark:text-slate-300 text-sm">
                  Mfg. Date
                </Label>
                <Input
                  id="mfgDate"
                  ...
                  className="rounded-xl dark:bg-slate-800 border-slate-200 dark:border-slate-700 focus-visible:ring-indigo-500 text-sm"
                />
              </div>
              <div className="space-y-1.5">
                <Label htmlFor="expiryDate" className="font-semibold text-slate-700 dark:text-slate-300 text-sm">
                  Expiry Date
                </Label>
                <Input
                  id="expiryDate"
