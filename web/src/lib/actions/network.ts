...
export async function getNetworkRegistry() {
  try {
    ...
    return items.map(item => ({
      ...
      stockQuantity: Number(item.stockQuantity?.toString() || 0),
      minStockLevel: Number(item.minStockLevel?.toString() || 0),
      status: item.status,
...
export async function sourceItem(productId: string) {
  try {
    ...
    return {
      ...
      stockQuantity: Number(newItem.stockQuantity?.toString() || 0),
      minStockLevel: Number(newItem.minStockLevel?.toString() || 0),
