          ...
          <form onSubmit={handleSubmitAndLogout} className="flex-1 flex flex-col min-h-0 overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto p-5 sm:p-6 space-y-4 custom-scrollbar">
              ...
              <div className="flex flex-col items-center justify-center p-4 rounded-2xl bg-slate-50 dark:bg-slate-900/60 border border-slate-200/80 dark:border-slate-800/80 space-y-3 shadow-inner">
                ...
                {/* Star Buttons */}
                <div className="flex items-center justify-center gap-1.5 sm:gap-2 py-0.5 select-none">
                  {[1, 2, 3, 4, 5].map((star) => {
                    ...
                    return (
                      <button
                        ...
                        className={`h-9 w-9 sm:h-10 sm:w-10 rounded-xl flex items-center justify-center transition-all cursor-pointer touch-manipulation focus:outline-none ${
                          active 
                            ? "bg-amber-400/15 border-2 border-amber-400 scale-105 shadow-sm shadow-amber-400/20" 
                            : "bg-slate-100 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 hover:scale-105"
                        }`}
                        aria-label={`${star} Stars`}
                      >
                        <Star
                          className={`h-4.5 w-4.5 sm:h-5 sm:w-5 pointer-events-none transition-colors ${
                            active
                              ? "text-amber-400 fill-amber-400 drop-shadow-[0_1px_6px_rgba(251,191,36,0.6)]"
                ...
                <div className="grid grid-cols-5 gap-1 w-full pt-0.5">
                  {[
                    ...
                    <button
                      ...
                      className={`py-1 px-0.5 rounded-lg text-[9px] sm:text-[10px] font-bold transition-all border text-center cursor-pointer ${
                        rating === item.val
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-1">
                ...
                <div className="space-y-1">
                  ...
                  <select
                    ...
                    className="h-8 w-full text-xs rounded-xl bg-background border border-input px-3 py-1 text-slate-900 dark:text-slate-200 focus:ring-2 focus:ring-indigo-500"
                  >
              ...
              <div className="flex items-center gap-2 p-2 rounded-xl bg-emerald-50 dark:bg-emerald-950/30 border border-emerald-200 dark:border-emerald-800/40 text-emerald-800 dark:text-emerald-300 text-[11px]">
            ...
            {/* Pinned Bottom Action Buttons (Never cut off & compact) */}
            <div className="shrink-0 p-3 sm:p-3.5 bg-slate-50/95 dark:bg-slate-900/95 backdrop-blur-md border-t border-slate-200/80 dark:border-slate-800/80 flex flex-row items-center justify-between gap-2">
              <Button
                type="button"
                variant="ghost"
                size="sm"
                ...
                disabled={submitting}
                className="h-8 sm:h-8.5 px-3 text-xs font-semibold text-slate-500 hover:text-slate-900 dark:hover:text-white cursor-pointer"
              >
                Skip &amp; Log Out
              </Button>
              <Button
                ...
                size="sm"
                disabled={submitting}
                className="h-8.5 sm:h-9 px-4 rounded-xl bg-gradient-to-r from-indigo-600 via-indigo-700 to-purple-700 hover:from-indigo-700 hover:to-purple-800 text-white font-bold text-xs uppercase tracking-wider gap-1.5 shadow-md shadow-indigo-600/20 cursor-pointer"
              >
                {submitting ? (
                  "Submitting..."
                ) : (
