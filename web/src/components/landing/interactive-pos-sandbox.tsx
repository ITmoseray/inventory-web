            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center gap-1.5 overflow-x-auto pb-1 custom-scrollbar">
                {categories.map((cat) => (
                        ...
                        {filteredProducts.map((product) => {
                          ...
                          return (
                            <motion.button
                              ...
                              className={`p-2.5 sm:p-3.5 rounded-2xl text-left transition-all border relative flex flex-col justify-between cursor-pointer group ${
                                inCartItem
                            ...
                            >
                              ...
                              <div>
                                <span className="text-xl sm:text-2xl block mb-1.5">{product.image}</span>
                                <h4 className="text-[11px] sm:text-xs font-bold text-white group-hover:text-indigo-300 transition-colors line-clamp-1">
                                  {product.name}
                                </h4>
                              ...
                              <div className="flex items-center justify-between mt-2.5 pt-2 border-t border-slate-700/40">
                                <span className="text-[11px] sm:text-xs font-black text-amber-400">Le {product.price.toLocaleString()}</span>
                                <div className="h-6 w-6 rounded-lg bg-indigo-600/20 group-hover:bg-indigo-600 text-indigo-400 group-hover:text-white flex items-center justify-center transition-colors shrink-0">
                  ...
                  <div className="lg:col-span-5 p-4 sm:p-6 bg-slate-950/80 flex flex-col justify-between">
                        ...
                        >
                          <div className="space-y-3">
                            <div className="text-center pb-3 border-b border-dashed border-slate-800 space-y-1">
                              ...
                              <h3 className="text-sm sm:text-base font-black text-white uppercase tracking-wider">Sale Complete &amp; Reconciled!</h3>
                              <p className="text-[10px] sm:text-[11px] text-slate-400">Order ID: {completedOrder.orderNumber} • {completedOrder.date}</p>
                            </div>
                            ...
                            <div className="space-y-1.5 max-h-48 overflow-y-auto custom-scrollbar pr-1 text-xs">
                              {completedOrder.items.map((item: CartItem) => (
                                <div key={item.product.id} className="flex justify-between items-center py-1 border-b border-slate-900">
                                  ...
                                  <span className="font-bold text-white shrink-0">Le {(item.product.price * item.quantity).toLocaleString()}</span>
                                </div>
                              ))}
                            </div>
                          ...
                          <div className="space-y-2 pt-3">
                            ...
                            <Link
                              href="/register"
                              className="w-full h-10 rounded-xl bg-gradient-to-r from-indigo-600 via-indigo-700 to-purple-700 hover:from-indigo-500 hover:to-purple-600 text-white font-black text-xs uppercase tracking-wider flex items-center justify-center gap-2 shadow-lg shadow-indigo-600/30 transition-all text-center"
                            >
                        ...
                        >
                          <div className="space-y-3">
                            <div className="flex items-center justify-between pb-2 border-b border-slate-800">
                              <div className="flex items-center gap-2">
                                <ShoppingCart className="h-4 w-4 text-indigo-400 shrink-0" />
                              ...
                              <div className="grid grid-cols-2 gap-1.5">
                                {[
                                  ...
                                  <button
                                    ...
                                    className={`p-2 rounded-xl text-left text-[10px] sm:text-[11px] font-bold border transition-all flex items-center gap-2 cursor-pointer ${
                          ...
                          <Button
                            ...
                            className="w-full h-11 rounded-xl bg-gradient-to-r from-emerald-600 via-teal-600 to-indigo-600 hover:from-emerald-500 hover:to-indigo-500 text-white font-black text-[11px] sm:text-xs uppercase tracking-wider sm:tracking-widest gap-2 shadow-lg shadow-emerald-600/20 cursor-pointer disabled:opacity-50"
                          >
                            {isProcessing ? (
                              "Processing Payment..."
                            ) : (
                              <>
                                ...
                                <span className="truncate">Complete Sale (Le {total.toLocaleString()})</span>
                              </>
                            )}
                          </Button>
                        </motion.div>
